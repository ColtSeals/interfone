...
build_asterisk(){
  ...
  # prereqs recomendados pelo próprio Asterisk (quando disponível)
  yes | contrib/scripts/install_prereq install || true

  ./configure
  # VPS/CPU virtual esquisita? isso ajuda a evitar binário que builda e não roda
  ...
  command -v asterisk >/dev/null 2>&1 || die "asterisk ainda não apareceu no PATH após install"
}
